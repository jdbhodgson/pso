...
class Swarm(object):
    ...
    def __init__(self, m, n, f, b_lo, b_up, w=0.9, phi_p=0.7, phi_g=0.7):
        ...
        self.n = n   # int - particle dimension
        ...
        # Ring topology
        self.groups = [[self.particles[(i+j)%m] 
                        for i in range(-1,2)]
                        for j in range(m)]
        self.update_swarm_g()
    ...
    def __repr__(self):
        out = ('<Swarm: particles=%d, minimizer=%s, best=%8.2e>'
                % (self.m, self.f.__name__, self.best[1]) )
        return out
...
class Particle(object):
    ...
    def __init__(self, swarm, j, w, phi_p, phi_g):
        ...
        self.x = [random.random()*swarm.range[i] 
                  + swarm.b_lo[i] for i in range(n)]
        self.v = [2*random.random()*swarm.range[i]
                    - swarm.range[i] for i in range(n)]
                    
        self.vmax = [0.2*swarm.range[i] for i in range(n)]
